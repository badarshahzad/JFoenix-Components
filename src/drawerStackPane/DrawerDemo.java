...
//		rightDrawer.setContent(bottomDrawer);
		/**
		 * This is setOvverlay visiable is to show lay over the pane
		 * */
		...
		*/
		...
		/*
		 if(transition.getRate()==1){ 
			...
			// drawersStack.setVisible(true); 
			 } 
		 else{ 
			 borderpane.setRight(null);
			 drawersStack.toggle(rightDrawer);
			 //drawersStack.setVisible(false); 
			 }
			 */
		...
		scene.getStylesheets().add(DrawerDemo.class.getResource("jfoenix-components.css").toExternalForm());
		scene.getStylesheets().add(DrawerDemo.class.getResource("jfoenix-design.css").toExternalForm());
